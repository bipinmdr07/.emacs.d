...
(use-package pyvenv
  :ensure t
  :config
  (pyvenv-mode 1)) ;; enables automatic pyenv tracking

(defun my/auto-activate-venv ()
  "Auto-activate Python virtualenv from project root if venv or .venv directory exists."
  (when (eq major-mode 'python-mode)
    (let* ((project-root (or (when (fboundp 'project-root)
                             ...
                             (locate-dominating-file default-directory ".venv")))
           (venv-path (cond
                       ((and project-root (file-directory-p (expand-file-name "venv" project-root)))
...
(use-package general
  ...
  (bipin/leader-keys
    ...
    "f j" '(consult-file :wk "Jump to a file below current directory")
    "f l" '(counsel-locate :wk "Locate a file")
    "f r" '(counsel-recent-file :wk "Find recent files")
    )
